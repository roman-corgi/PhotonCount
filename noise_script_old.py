...
if __name__ == '__main__':
    ...
    fluxmap = np.ones((pix_row,pix_row))#*.5 #photon flux map, photons/s
    frametime = .05#.1  # s (adjust lambda by adjusting this)
    em_gain = 5000. # e-
    ...
    master_dark  = np.mean(master_dark_list, axis=0)
    ...
    # def prob_dist(x, g, L, T, N):
    #     return (((1 - eThresh(L, g, T))**(N - x)*
    #              eThresh(L, g, T)**x*gamma(1 + N))/(gamma(1 + N - x)*gamma(1 + x)))
#XXX make large N approx for this for N>150?
    ...
    def prob_dist(x, g, L, T, N):
        return (comb(N-1, x)+comb(N-1, x-1))*((1 - eThresh(L, g, T))**(N - x)*
                 eThresh(L, g, T)**x)
    ...
    if md:
        frames_unc = get_counts_uncorrected(grand_e_cube - master_dark,
